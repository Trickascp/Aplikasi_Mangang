...
module.exports = { 
    ...
    getlistcartall: (req, res) => {
        ...
        JOIN negara n on p.id_negara = n.id
        JOIN users u on u.id = k.id_agent
        LEFT JOIN local_deli ld on ld.negara = n.id
        WHERE u.username='${req.params.username}';`;
        conn.query(sql, (err, result) => {
            if(err) throw err;
            console.log(sql);
    ...
    editPaket: (req, res) => {
        var { id,nama,harga,price, disc} = req.body;
        
        var sql = `UPDATE paket SET nama = '${nama}', harga = ${harga}, price = '${price}', disc = '${disc}'
                    WHERE id = ${id};`;
        conn.query(sql, (err, result) => {
            if(err) throw err;
            console.log(sql);
    ...
    getCustPjt: (req, res) => {
     ...
     getLocalShipping: (req, res) => {

        var sql = `SELECT * FROM local_deli ld
        LEFT JOIN negara n on n.id = ld.negara `
     ...
     deliEdit: (req, res) => {
        ...
        var sql = `UPDATE local_deli SET status=${req.body.stat} WHERE id = ${req.body.id}`
     ...
     updateDeli: (req, res) => {
        ...
        var { jakarta, w_java, e_java, c_java, oth, id } = req.body

        var sql = `UPDATE local_deli SET dki_Jakarta = ${jakarta}, 
        west_java = '${w_java}',
        east_java = '${e_java}',
        central_java = '${c_java}',
        other = '${oth}'
