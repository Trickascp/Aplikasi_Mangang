...
class CartAgent extends Component {
    ...
    renderDeli = (item) => {
        ...
        if(item.status == 1){
            if(item.provinsi == "West Java"){
                return this.state.listCart[0].west_java
            }else if(item.provinsi == "East Java"){
                return this.state.listCart[0].east_java
            }else if(item.provinsi == "Central Java"){
                return this.state.listCart[0].central_java
            }else if(item.provinsi == "DKI Jakarta"){
                return this.state.listCart[0].dki_Jakarta
            }else{
                return this.state.listCart[0].other
            }
        }else{
            return "0"
        }
            ...
            if(item.id_customer !== this.state.selectedRow){
                ...
                return (
                    <tr key={item.id_customer} className="text-wrap" style={{fontSize:'12px'}}>  
                        ...
                        <td className="align-middle">{this.renderDeli(item)}</td>
    ...
    ringkasanBelanja = (limit) => {
       ...
       var deli = 0
        for(var i = 0; i < listCart.length; i++){
            ...
            if(this.state.listCart[i].status == 1){
                if(listCart[i].provinsi == "West Java"){
                    deli += Number(listCart[0].west_java)
                }else if(listCart[i].provinsi == "East Java"){
                    deli += Number(listCart[0].east_java)
                }else if(listCart[i].provinsi == "Central Java"){
                    deli += Number(listCart[0].central_java)
                }else if(listCart[i].provinsi == "DKI Jakarta"){
                    deli += Number(listCart[0].dki_Jakarta)
                }else{
                    deli += Number(listCart[0].other)
                }
            }else{
                deli += 0
            }
