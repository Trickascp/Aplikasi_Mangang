...
class ManageAgent extends Component{
    ...
    banUser = (id, level) => {
        ...
        if(level !== "BAN"){
            ...
            var valid = window.confirm("Are you sure want to ban this account?")

            if(valid){
                
                axios.post(`${KONEKSI}/auth/banUser`, {
                    id: id,
                    role: 'BAN'
                }).then((res) => {
                    alert("User Banned")
            ...
            var valid = window.confirm('Are you sure want unbanned this account?')
            if(valid){
                axios.post(`${KONEKSI}/auth/banUser`, {
                    id: id,
                    role: 'User'
    ...
    renderButton = (item) => {
        ...
        if(item.role !== 'BAN'){
            return (
                <td>
                    <button className="btn btn-warning mb-1" onClick={() => this.renderModal2(item)}><i className="fa fa-edit"></i></button>
                    <button className="btn btn-danger" onClick={() => this.banUser(item.id_agent, item.role)}><i className="fa fa-ban"></i></button>
                </td>
            )
        }else{
            return (
                <td>
                    <button className="btn btn-success" onClick={() => this.banUser(item.id_agent, item.role)}>unban</button>
                </td>
            )
        }
