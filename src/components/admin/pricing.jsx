...
class Pricing extends Component {
    ...
    onBtnSavePaket = (id) => {
        ...
        var disc = this.refs.editDisc.value
            ...
            return (
                <tr key={item.isbn} className="text-wrap"  style={{fontSize:'12px'}}>                    
                   ...
                   <td className="align-middle"><input type="text" ref="editNama" placeholder="Nama paket" className="form-control form-control-sm" defaultValue={item.nama} /></td>
                    ...
                    <td className="align-middle"><input type="text" ref="editDisc" placeholder="discount" className="form-control form-control-sm" defaultValue={item.disc} /></td>
    ...
    getLocalShipping = () => {
        ...
        axios.get(`${KONEKSI}/transaction/getLocalShipping`)
        .then((res) => {
            this.setState({deli: res.data})
        }).catch((err) => {
            console.log(err)
        })
    ...
    deliEdit = (id, stat) => {
        ...
        axios.post(`${KONEKSI}/transaction/deliEdit`,{
            id: id, 
            stat: stat
        }).then((res) => {
            this.getLocalShipping()
    ...
    buttonDeli = (item) => {
        ...
        if(item.status == '1'){
            return (
                <button className="btn btn-danger ml-1 mb-1 btn-sm" onClick={() => this.deliEdit(item.id, 2)}><i className="fa fa-close"></i></button>
            )
        }else{
            return (
                <button className="btn btn-success ml-1 mb-1 btn-sm" onClick={() => this.deliEdit(item.id, 1)}><i className="fa fa-check"></i></button>
            )
        }
    ...
    updateDeli = (id) => {
        ...
        var jakarta = this.refs.jakarta.value
        var w_java  = this.refs.w_java.value
        var e_java  = this.refs.e_java.value
        var c_java  = this.refs.c_java.value
        var oth     = this.refs.oth.value

        axios.post(`${KONEKSI}/transaction/updateDeli`,
        {
            jakarta: jakarta,
            w_java: w_java,
            e_java: e_java,
    ...
    tableLocalDeli = () => {
        ...
        var { deliEdit } = this.state

        const data = this.state.deli.map((item) => {

            if(item.id !== this.state.deliEdit){
                return (
                    <tr key={item.id} style={item.status == '1' ? {} : {backgroundColor: "#f8d7da"}}>
                        <td>{item.kode}</td>
                        <td>{item.dki_Jakarta}</td>
                        <td>{item.west_java}</td>
                        <td>{item.east_java}</td>
                        <td>{item.central_java}</td>
                        <td>{item.other}</td>
                        <td><input type="checkbox" checked={item.status == '1' ? "on" : ""} disabled className="form-control"/></td>
                        <td>{this.buttonDeli(item)}
                        <button className="btn btn-warning ml-1 mb-1 btn-sm" onClick={() => this.setState({deliEdit: item.id})}><i className="fa fa-edit"></i></button></td>
                    </tr>
                )
            }else{
                return (
                    <tr key={item.id} style={item.status == '1' ? {} : {backgroundColor: "#f8d7da"}}>
                        <td>{item.kode}</td>
                        <td><input type="number" className="form-control" ref="jakarta" defaultValue={item.dki_Jakarta}/></td>
                        <td><input type="number" className="form-control" ref="w_java" defaultValue={item.west_java}/></td>
                        <td><input type="number" className="form-control" ref="e_java" defaultValue={item.east_java}/></td>
                        <td><input type="number" className="form-control" ref="c_java" defaultValue={item.central_java}/></td>
                        <td><input type="number" className="form-control" ref="oth" defaultValue={item.other}/></td>
                        <td><input type="checkbox" checked={item.status == '1' ? "on" : ""} disabled className="form-control"/></td>
                        <td><button className="btn btn-success ml-1 mb-1 btn-sm" onClick={() => this.updateDeli(item.id)}><i className="fa fa-check"></i></button>
                        <button className="btn btn-secondary ml-1 mb-1 btn-sm" onClick={() => this.setState({deliEdit: 0})}><i className="fa fa-close"></i></button></td>
                    </tr>
                )
            }
                    ...
                    <div className="row">
                        ...
                        <div className="col-md-10 bg-light pl-3 pt-3">
                                ...
                                <div className="row justify-content-sm-left mt-3 ml-1 text-left text-secondary" style={{fontSize:"14px"}} >
                                    ...
                                    <h6 className="text-secondary text-left">Manage Local Delivery</h6>
                                    <table className="table table-hover text-secondary text-center" style={{fontSize:"12px"}}>
                                        <thead className="blue text-white">
                                            <th>Negara</th>
                                            <th>DKI Jakarta</th>
